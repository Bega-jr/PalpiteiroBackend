...
def main():
    ...
    try:
        ...
        if dispersao >= 5:
            ...
            print(
                "⚠️ Instabilidade contextual "
                "detectada. "
                "Forçando CONTRACAO_FRIAS."
            )
        ...
        payload_regime = {
            ...
            "contexto_repetidos":
                float(
                    np.mean([
                        len(
                            set(historico[i]["numeros"])
                            &
                            set(historico[i - 1]["numeros"])
                        )

                        for i in range(
                            1,
                            min(15, len(historico))
                        )
                    ])
                ),
        ...
        print(
            f"📡 Regime adaptativo consolidado: "
            f"{regime} | "
            f"Score Global: {media_score:.4f}"
        )
