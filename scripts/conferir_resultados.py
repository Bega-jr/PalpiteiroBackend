...
def conferir_historico():
    ...
    if not res_oficiais.data: return
    ...
    for sorteio in res_oficiais.data:
        ...
        if not palpites_res.data: continue
        ...
        for p in palpites_res.data:
            ...
            if tipo not in resumo_por_tipo:
                resumo_por_tipo[tipo] = {"11":0, "12":0, "13":0, "14":0, "15":0, "qtd_total": 0}
        ...
        for tipo, dados in resumo_por_tipo.items():
            ...
            try:
                # 💥 AQUI: Deleta a linha antiga primeiro
                supabase.table("palpites_resultados_reais").delete()\
                    .eq("concurso_inicio", concurso_id)\
                    .eq("tipo_palpite", tipo).execute()

                # ✅ E depois insere a nova linha
                supabase.table("palpites_resultados_reais").insert(registro).execute()
                print(f"✅ Concurso {concurso_id} [{tipo}] atualizado com sucesso.")
            except Exception as e:
                print(f"❌ Erro ao gravar/atualizar concurso {concurso_id}: {e}")
