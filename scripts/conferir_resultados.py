...
def carregar_memoria(supabase):
    ...
    registros = supabase.table("memoria_cenarios") \
        .select("*") \
        .execute()
...
def atualizar_memoria_local(memoria_cache, nums, acertos):
    ...
    novo_score = (
        ((score_antigo * (vezes - 1)) + peso)
        / vezes
    )

    mem["score_medio_real"] = round(novo_score, 4)
...
def sincronizar_memoria(supabase, memoria_cache):
    ...
    if inserts:
        print(f"➕ Inserindo {len(inserts)} novos cenários")

        for i in range(0, len(inserts), 50):
            supabase.table("memoria_cenarios") \
                .insert(inserts[i:i+50]) \
                .execute()
...
def main():
    ...
    print("🏁 [v5.7-STABLE] Conferência Inteligente iniciada")

    # ==================================================
    # RESULTADOS OFICIAIS
    # ==================================================

    oficiais = supabase.table("lotofacil_concursos") \
    ...
    for item in oficiais.data:

        nums = parse_numeros(item["dezenas"])

        if nums:
            resultado_map[int(item["concurso"])] = set(nums)

    print(f"📊 {len(resultado_map)} concursos carregados")

    # ==================================================
    # MEMÓRIA
    # ==================================================

    memoria_cache = carregar_memoria(supabase)

    # ==================================================
    # PENDENTES
    # ==================================================

    pendentes = supabase.table("palpites_validos") \
        .select("*") \
        .eq("processado", False) \
        .execute()
    ...
    for p in pendentes:
        ...
        try:
            ...
            if not nums:
                print(f"⚠️ Palpite inválido ID={p['id']}")
                continue

            acertos = len(
                set(nums) &
                resultado_map[concurso]
            )
            ...
            print(f"⚠️ Erro processando ID={p.get('id')}: {e}")
    ...
    if updates_palpites:
        ...
        print(f"🔄 Atualizando {len(updates_palpites)} palpites")
    ...
    todos = supabase.table("palpites_validos") \
        .select(
            "data_referencia,"
            "concurso_referencia,"
            "tipo,"
    ...
    print(f"🚀 Enviando {len(items)} grupos")
